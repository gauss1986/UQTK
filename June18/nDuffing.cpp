...
int main(int argc, char *argv[]){
    ...
    Array2D<double> performance(4,3,0.e0); // performance matrix to output the GS1,AAPG1,AAPG2
    ...
    for (int ord=1;ord<=ord_GS;ord++){
        ...
        performance(1,0) = e3(0);
        performance(1,1) = e3(1);
        performance(1,2) = time(1);
    ...
    cout << "AAPG..." << endl;
    nAAPG(refine,dof,nkl,dim,nStep,ord_AAPG_GS,lout,factor_OD,ord_AAPG,act_D,fbar,fbar_fine,dTym,epsilon_mean,pcType,scaledKLmodes,scaledKLmodes_fine,stat_e,stat_i,stat_m,stat_c,stat_k,normsq,mean_MCS,std_MCS,mck,PDF,samPts_norm,active_D,p,performance);
