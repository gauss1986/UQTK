...
Array1D<double> AAPG(int dof, Array1D<double> inpParams, Array1D<double>& fbar, double dTym, int order, string pcType, int noutput, int dim, int nStep, Array2D<double>& scaledKLmodes, Array1D<double>& normsq, double factor_OD, int AAPG_ord, bool act_D, double p, Array2D<double>& mstd_MCS, FILE* err_dump, Array2D<double>& sample_mstd_2D, Array2D<double>& samPts_norm, Array2D<double>& e_AAPG, Array1D<Array1D<double> >& e_sample_dis, Array1D<Array1D<double> >& e_sample_vel, Array1D<int>& init_D, Array1D<int>& coeff_D, bool PDF){
    ...
    printf("Dis...\n");
    tt.tick();
    PostProcess(indi_2,indj_2, indi_3, indj_3, indk_3, AAPG_ord, dis_0, dis_1, dis_2, dis_3, dis_1_mean, dis_2_mean, dis_3_mean, std1, std2, std3,  normsq, dim, nStep, PCTerms_1, PCTerms_2, PCTerms_3, order, dTym, factor_OD, mstd_MCS, err_dump, samPts_norm, name, noutput, e_sample_dis, PDF);
    tt.tock("Took");
    t(4)=tt.silent_tock();
    printf("Vel...\n");
    string name2 = "vel";
    ...
    tt.tick();
    PostProcess(indi_2,indj_2, indi_3, indj_3, indk_3, AAPG_ord, vel_0, vel_1, vel_2, vel_3, vel_1_mean, vel_2_mean, vel_3_mean, std_vel_1, std_vel_2, std_vel_3,  normsq, dim, nStep, PCTerms_1, PCTerms_2, PCTerms_3, order, dTym, factor_OD, mstd_MCS, err_dump, samPts_norm, name2, noutput, e_sample_vel, PDF);
    tt.tock("Took");
    t(5)=tt.silent_tock();
...
void PostProcess(Array1D<int>& indi_2, Array1D<int>& indj_2, Array1D<int>& indi_3, Array1D<int>& indj_3, Array1D<int>& indk_3, int AAPG_ord, Array1D<double>& dis_0, Array1D<Array2D<double> >& dis_1, Array2D<Array2D<double> >& dis_2, Array3D<Array2D<double> >& dis_3, Array1D<double>& dis_1_mean, Array1D<double>& dis_2_mean, Array1D<double>& dis_3_mean, Array1D<double>& std1, Array1D<double>& std2, Array1D<double>& std3, Array1D<double>& normsq, int dim, int nStep, int PCTerms_1, int PCTerms_2, int PCTerms_3, int order, double dTym, double factor_OD, Array2D<double>& mstd_MCS, FILE* err_dump, Array2D<double>& samPts_norm, string name,int noutput, Array1D<Array1D<double> >& e_sample, bool PDF){
    TickTock tt;
    tt.tick();
    ...
    tt.tick();
    computeStd(nStep, nPCTerms, dis_1_assembled,dis_2_assembled, dis_3_assembled, normsq, std1, std2, std3);
